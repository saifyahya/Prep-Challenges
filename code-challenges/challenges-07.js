...
const cvFormatter = (arr) => {
    ...
    for (let i =0 ; i< arr.length;i++) {
        ...
        myArray.push({fullName:arr[i].firstName+" "+arr[i].lastName,tech:arr[i].tech})
        if(arr[i].lastName==null)
        myArray.push({fullName:arr[i].firstName,tech:arr[i].tech})
    }}
    return myArray
};
// -------------------------------------------------------------------------------------------------------
...
const applicationsStatics = (arr) => { 
  ...
  totalApplicants: 0,
    ...
    for (let i =0 ; i< arr.length;i++) {
if(arr[i].yearsOfExperience<=1 || arr[i].firstName=="" && arr[i].lastName=="" || arr[i].firstName==null && arr[i].lastName==null)
...
const classesAvg = (data) => {
    let sum=0;
for(let i =0 ;i<data.grades.length;i++ ) {           //iterate over the grades array of objects
    for (let j=0 ;j< data.grades[i].classes.length;j++ ){ //iterate over each class in the grade
